...
public class BST<T extends Comparable<T>> {
   ...
   public BST() {
    ...
    public T getRoot() throws NoSuchElementException{
        if(isEmpty()) {
        	throw new NoSuchElementException("getRoot: BSTT is empty!");
        }
    	return root.data;
    }
    ...
    private int getHeight(Node node) {
    	if (node == null) {
    		...
    		if(leftHeight > rightHeight) {
    			...
    			return rightHeight + 1; // leftHeight = rightHeight case covered here
    		}
    	}
    }
    ...
    private boolean search(T data, Node node) {
    	if(data.compareTo(node.data) == 0/*if they are equals, compare to will return 0*/) {
    		return true;
    	}else if(data.compareTo(node.data) < 0/*if data is larger than node*/){
    		if(node.left == null) {
    			return false;
    		}else {
    			return search(data, node.left);
    		}
    	}else /*if data is smaller than node data*/{
    		if(node.right == null) {
    			return false;
    		}else {
    ...
    private boolean equals(Node node1, Node node2) {
        if((node1 ==null) &&(node2 ==null)) {
        	return true;//Two BST are identical or both of them are empty;
        }
    ...
    private void insert(T data, Node node) {
       if (node == null) {
    	   ...
    	   //what about case where the data is equal to the root?
    ...
    public void remove(T data) throws NoSuchElementException{
      if(isEmpty()) {
    	  throw new NoSuchElementException("remove: data doesn't exist!");
      }
      else if(!search(data)) {
    	  throw new NoSuchElementException("remove: data doesn't exist!");
      }
    ...
    private Node remove(T data, Node node) {  
    	/*looking for the reference node*/
    	if (node == null) {
    		return node;
    	}
    	...
    	/*after found, it skips the searching part and enters removing phase here*/
    	else {
